...
export default function Hero(){
 const content=useContent();const preference=useReducedMotion();const ref=useRef<HTMLElement>(null);const inView=useInView(ref);const reduced=preference||!content.motion.enabled;
 ...
 const objectY=useTransform(scrollYProgress,[0,1],[0,-100*content.motion.parallax]);const textY=useTransform(scrollYProgress,[0,1],[0,-35*content.motion.parallax]);
 ...
 return <section className="hero" id="top" ref={ref}>
  <Navigation/>
  <div className="hero-note"><span className="cross">+</span> <span className="editable-lines">{content.text.heroNote}</span></div>
  <motion.h1 className="hero-title" style={{y:reduced?0:textY}}><span>{content.text.heroLine1}</span><span>{content.text.heroLine2}<span className="title-star">✳</span></span></motion.h1>
  ...
  <div className="hero-caption"><span className="editable-lines">{content.text.heroCaption}</span><a href="#work">EXPLORE SELECTED WORK <span>↓</span></a></div>
