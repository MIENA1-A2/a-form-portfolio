...
export function PageShell({children}:{children:ReactNode}){
 const [cover,setCover]=useState<Cover|null>(null);const router=useRouter();const pathname=usePathname();const reduced=useReducedMotion();
 ...
 return <NavigateContext.Provider value={navigate}><LayoutGroup><a className="skip-link" href="#main-content">Skip to content</a>{children}<AnimatePresence>{cover&&<motion.div className="route-cover" initial={{opacity:1}} exit={{opacity:0}} transition={{duration:.25}} aria-hidden="true"><motion.img src={assetPath(cover.project.image)} alt="" initial={cover.rect} animate={{top:0,left:0,width:"100vw",height:"100vh"}} transition={{duration:.65,ease:[.22,1,.36,1]}} onAnimationComplete={()=>{if(pathname===cover.from)router.push("/work/"+cover.project.slug,{scroll:true})}}/></motion.div>}</AnimatePresence></LayoutGroup></NavigateContext.Provider>
}
...
export function Reveal({children,className=""}:{children:ReactNode;className?:string}){
 ...
 return <motion.div className={className} initial={false} whileInView={reduced?{opacity:1,y:0}:{opacity:[.25,1],y:[30,0]}} viewport={{once:true,amount:.12}} transition={{duration:reduced?0:.85,ease:[.22,1,.36,1]}}>{children}</motion.div>
}
