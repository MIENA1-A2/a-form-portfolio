...
import {projects as baseProjects,profile,type Project} from "./data";
...
function Work({project}:{project:Project}){
 ...
 return <section className={"work work-"+project.theme} id={project.slug}>
  ...
  <ProjectLink project={project} className="work-image">
   <motion.div initial={false} whileInView={reduced?{}:{clipPath:["inset(8% 0 8% 0)","inset(0% 0 0% 0)"]}} viewport={{once:true,amount:.15}} whileHover={reduced?{}:{scale:content.motion.hoverScale}} transition={{duration:content.motion.duration,ease:[.22,1,.36,1]}}><Artwork project={project}/></motion.div>
...
export default function HomeContent(){
 ...
 return <main id="main-content"><Hero/>
  ...
  <section className="about" id="about"><div className="eyebrow">A / FORM — THE PRACTICE <span>INDEPENDENT BY DESIGN</span></div><Reveal><h2 className="editable-lines">{content.text.aboutHeading}</h2></Reveal><div className="about-description"><span className="about-symbol" aria-hidden="true">a/f</span><div><p>{content.text.aboutBody}</p><p>{content.text.aboutSecond}</p><span className="concept-note">A fictional practice. Real curiosity.<br/>All projects shown are self-initiated concept studies.</span></div></div><dl className="profile"><div><dt>LOCATION</dt><dd>{profile.location}</dd></div><div><dt>FOCUS</dt><dd>Brand identity<br/>Art direction<br/>Digital experience</dd></div><div><dt>PRACTICE</dt><dd>Independent<br/>Concept studies / 2026</dd></div><div><dt>CONTACT</dt><dd>Details coming soon<br/><span>Demonstration portfolio</span></dd></div></dl></section>
  <section className="contact" id="contact"><div className="eyebrow">GOOD THINGS BEGIN WITH A CONVERSATION <span>LET’S FIND THE UNEXPECTED.</span></div><Reveal><h2><span className="editable-lines">{content.text.contactHeading}</span><span className="contact-arrow">↗</span><span className="contact-period">.</span></h2></Reveal><div className="contact-bottom"><p className="editable-lines">{content.text.contactBody}</p><div className="contact-channels" aria-label="Contact channels are not yet available"><span>EMAIL <sup>SOON</sup></span><span>INSTAGRAM <sup>SOON</sup></span><span>BEHANCE <sup>SOON</sup></span><span>LINKEDIN <sup>SOON</sup></span></div></div></section>
  <Footer/>
 </main>
}
