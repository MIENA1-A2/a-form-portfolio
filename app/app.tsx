...
function Route(){
 ...
 if(path==='/')return <Published pageId="home"/>;
 if(project)return <Published pageId={project.slug}/>;
 if(path==='/studio-next'||path==='/studio')return <Suspense fallback={<div className="app-loading">Opening Visual Studio…</div>}><Workbench/></Suspense>;
 return <main className="not-found" id="main-content"><p>404</p><h1>THIS PAGE<br/>IS OUT OF FRAME.</h1><a href={assetPath('/')}>RETURN HOME →</a></main>;
}
